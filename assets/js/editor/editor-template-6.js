const userId = JSON.parse(decrypt(localStorage.getItem(encrypt('userData', localStorage.getItem(btoa('sessionId_' + window.location.origin)))), localStorage.getItem(btoa('sessionId_' + window.location.origin)))).userId;
const userStatus = localStorage.getItem('isPremiumUser');
let resumeObj = JSON.parse(decrypt(localStorage.getItem(encrypt('userData', localStorage.getItem(btoa('sessionId_' + window.location.origin)))), localStorage.getItem(btoa('sessionId_' + window.location.origin))));
...
$(window).ready(function () {
    ...
    function ratingSkillCircle() {
        ...
        $(".skills-section .rating-circle").hover(
            ...
            function () {
                ...
                index = parseInt($(this).parent().parent().attr('data-skillpercentage')) / 25;
    ...
    function ratingCircle() {
        ...
        $(".language-section .rating-circle").hover(
            // When the mouse hover over any circle. All the circles to the left change color to yellow
            function () {
                $(this).removeClass("rating-chosen").addClass("rating-hover");
                $(this).prevAll().removeClass("rating-chosen").addClass("rating-hover");
        ...
        $(".language-section .rating-circle").click(
            function () {
                $(this).addClass("rating-chosen");
                $(this).prevAll().addClass("rating-chosen");
                // Remember the position of the click so it can be retrieved later
    ...
    for (let index = 1; index < skillsLength; index++) {
        ...
        $div.after($klon);
        var minSkill = '75%';
        $('#skills_item_' + num + ' .rating-container').attr('data-skillpercentage', minSkillValue);
        $('#skills_item_' + num + ' .skill-input-value').text('');
    ...
    $("body").on("click", ".add_skills", function () {
        if ($('.skills-item').length < 16) {
            ...
            $('#skills_item_' + num + ' .rating-container li').removeClass('rating-chosen');
            $('#skills_item_' + num + ' .rating-container li:first').addClass('rating-chosen').next().addClass('rating-chosen');
            // console.log('i am add_exp')
            $('#skills_item_' + num + ' .skill-name').keypress(function (e) {
                return limitLines($(this), LINE_SKILL_NAME, e);
            });
    ...
    function getExperinceDetails() {
        ...
        $('#resume-body .professional-experience').each(function (i, v) {
            ...
            var exp_startDate = $('#resume-body #' + id).find('input[data-content="exp_startDate"]')
                .val();
            var exp_jobTitle = ($('#resume-body #' + id).find('div[data-content="exp_jobTitle"]').attr(
                        'placeholder') != $('#resume-body #' + id).find('div[data-content="exp_jobTitle"]')
                    .text()) ?
                $('#resume-body #' + id).find('div[data-content="exp_jobTitle"]').text() : '';
    ...
    function getSkillsDetails() {
        ...
        $('#resume-body .skills-item').each(function (i, v) {
            ...
            var skill_value = parseInt($('#resume-body #' + id).find('.rating-container').attr('data-skillpercentage'));
    ...
    function bindUserExpData() {
        ...
        $(resumeObj.userExperienceResSet).each(function (i, v) {
            ...
            $('#resume-body #' + id).find('input[data-content="exp_startDate"]').val(resumeObj
                .userExperienceResSet[i]
                .startDate);
            $('#resume-body #' + id).find('div[data-content="exp_jobTitle"]').text(resumeObj
                .userExperienceResSet[i]
                .jobTitle);
    ...
    function bindUserSkillsData() {
        ...
        $(resumeObj.userSkillsSet).each(function (i, ev) {
            ...
            $('#skills_item_' + a + ' .rating-container').attr('data-skillpercentage', v.percentage);
            $('#skills_item_' + a + ' .skill-input-value').text(v.title);
            $('#skills_item_' + a + ' .skill').attr('data-content', '').text(v.title);
            selectSkillRatingCircle(a, v.percentage);
    ...
    function selectSkillRatingCircle(a, percentage) {
        $('#skills_item_' + a + ' .rating-container li.rating-circle').removeClass('rating-chosen');
        switch (percentage) {
            case 25:
                $('#skills_item_' + a + ' .rating-container .rating-circle:first-child').addClass(
                    'rating-chosen')
                break;
            case 50:
                $('#skills_item_' + a + ' .rating-container li.rating-circle:first-child').addClass(
                    'rating-chosen')
                $('#skills_item_' + a + ' .rating-container li.rating-circle:nth-child(2)').addClass(
                    'rating-chosen')
            ...
            case 75:
                ...
                $('#skills_item_' + a + ' .rating-container li.rating-circle:nth-child(2)').addClass(
                    'rating-chosen')
                $('#skills_item_' + a + ' .rating-container li.rating-circle:nth-child(3)').addClass(
                    'rating-chosen')

                break;
            case 100:
                $('#skills_item_' + a + ' .rating-container li.rating-circle').addClass('rating-chosen')
                break;
            default:
                break;
        }
    ...
    var renderPages = function () {
        ...
        var emptyDom = "<page size='a4'>\
                            <div class='row'>\
                                ...
                                <div class='col-9'>";
        var sideDom = "<div class='row'>\
                                " + getOuterHTML($('#resume-side')) +
            "<div class='col-9'>";

        var pageDom = "<page size='a4'>";
        ...
        var skillsMarginTop = parseFloat($('#skills-section').css('margin-top'));
        ...
        var languageHeight = parseFloat($('#language-section').css('height'));
        var languageMarginTop = parseFloat($('#language-section').css('margin-top'));
        ...
        console.log("Skills Margin Top: ", skillsMarginTop);
        ...
        var sideHeight = stackedHeight + memoHeight;

        console.log("Side Height : ", sideHeight);
        tempDom = pageDom + getOuterHTML($("#resume-title")) + "<div class='row'><div class='col-3'>";
        tempDom += getOuterHTML($("#resume-memo"));
        if (sideHeight + skillsHeight + skillsMarginTop <= standardA4Height) {
            tempDom += getOuterHTML($("#skills-section"));
            sideHeight += skillsHeight + skillsMarginTop;
        } else {
            sideHeight += parseFloat($('#skills-section h4').css('height')) + parseFloat($('#skills-section h4').css('margin-bottom')) + skillsMarginTop;
            var domHtml = '';
            var skillCount = $('#skills-section .skills-item').length;
            var skills = $('#skills-section .skills-item');
            var it;
            console.log("Skills Count: ", skillCount);
            domHtml += '<div class="row flex-column  px-1 mt-3 skill-row">\
            ...
            tempDom += domHtml + '</div><div class="col-9">';

            sideDoms.push(tempDom);

            domHtml = '<div class="row flex-column px-1 skill-row">\
                            <div class="skills-section">\
                                <ul class="list-unstyled w-100 skills-list mb-2">';
            ...
            tempDom = "<page size='a4'><div class='row'><div class='col-3'>" + domHtml;
        }

        if (sideHeight + languageHeight + languageMarginTop <= standardA4Height) {
            tempDom += getOuterHTML($('#language-section')) + '</div><div class="col-9">';
            sideDoms.push(tempDom);
        } else {
            sideHeight += parseFloat($('#language-section h4').css('height')) + parseFloat($('#language-section h4').css('margin-bottom')) + languageMarginTop;
            var domHtml = '';
            var langCount = $('#language-section .language-item').length;
            var langs = $('#language-section .language-item');
            var it;
            console.log("Languages Count: ", langCount);
            domHtml += '<div class="row flex-column  px-1 mt-3">\
            ...
            domHtml = '<div class="row flex-column px-1">\
                            <div class="language-section">\
                                <ul class="list-unstyled w-100 skills-list mb-2">';
            ...
            tempDom = "<page size='a4'><div class='row'><div class='col-3'>" + domHtml + '</div><div class="col-9">';
            sideDoms.push(tempDom);
        }
        ...
        if (stackedHeight + experienceHeight + experienceMarginTop <= standardA4Height) {
            ...
            resume.after(sideDoms[pageIndex++] + getOuterHTML($('#experience-section')) + emptyDomClose);
        } else {
            ...
            var it;
            var domHtml = '<div class="row mt-3 flex-column px-2">\
            ...
            resume.after(sideDoms[pageIndex++] + domHtml + emptyDomClose);

            domHtml = '<div class="row flex-column mb-1 px-2">';
